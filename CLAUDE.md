...
**Configurable appearance (Darstellung group on `passivmitgliedschaftElement`)**: two element properties let an editor override the look from the Umbraco backoffice without a redeploy:
...
The block partial reads both, resolves the media URL, and forwards them to the iframe as `?bg=` / `?line=` query parameters. `FloorPlanController` **sanitises** them (URL must be root-relative or `http(s)`; colour must match a `#hex` / `rgb[a](...)` whitelist, because `line` is rendered into a CSS context and Razor does not encode CSS) before passing them to the component as `BackgroundUrl` / `LineColor` parameters. Empty/invalid values fall back to the defaults (`/img/hallenboden.png` and the CSS `.pm-cell--free` stroke). The properties are defined in uSync schema (`ContentTypes/passivmitgliedschaftelement.config`, `DataTypes/EyeDropperLineColor.config`) **and** ensured on existing installs by `ContentSeeder.EnsurePassivmitgliedschaftElementProperties` (prod may run with uSync `ImportOnStartup` off).
